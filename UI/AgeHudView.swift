...
struct AgeHudView: View {
    ...
    func format(_ format: Format) -> String {
        switch format {
        case .maxAge:       return String(format: "% .2d", Census.shared.censusAgent.stats.maxAge)
        case .highwaterAge: return String(format: "% .2d", Census.shared.highwaterAge)
        case .averageAge:   return String(format: "% .2d", Census.shared.censusAgent.stats.averageAge)
