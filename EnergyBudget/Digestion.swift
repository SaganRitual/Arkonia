extension Metabolism {
    func digest() {
        ...
        for id in secondaryStores {
            guard let source = embryo.selectStore(id),
                let sink = self.selectStore(id) else { fatalError() }

            Debug.log(level: 179) { "digest.1a: \(source.E.organID):\(source.E.chamberID) level \(source.level) \(sink.E.organID):\(sink.E.chamberID) level \(sink.level)" }

            let (netDraw, netDeposit, drawFullness, depositFullness) =
                getTransferLevels(from: source, to: sink)

            Debug.log(level: 179) { "digest.1b: netDraw \(netDraw) netDeposit \(netDeposit) drawFullness \(drawFullness) depositFullness \(depositFullness)" }

            // Note: embryo never overflows to fat store
            transfer(netDraw, from: source, as: netDeposit, to: sink)

            Debug.log(level: 179) { "digest.1c: source result \(source.level) sink result \(sink.level)" }
        }
        ...
        for id in secondaryStores {
            guard let source = stomach.selectStore(id),
                  let sink = self.selectStore(id) else { continue } // Stomach doesn't have a fat store

            Debug.log(level: 179) { "digest.2a: \(source.E.organID):\(source.E.chamberID) level \(source.level) \(sink.E.organID):\(sink.E.chamberID) level \(sink.level)" }

            let (netDraw, netDeposit, drawFullness, depositFullness) =
                getTransferLevels(from: source, to: sink)

            Debug.log(level: 179) { "digest.2b: netDraw \(netDraw) netDeposit \(netDeposit) drawFullness \(drawFullness) depositFullness \(depositFullness)" }

            // Note: transfer doesn't overflow energy store to fat store
            transfer(netDraw, from: source, as: netDeposit, to: sink)

            Debug.log(level: 179) { "digest.2c: source result \(source.level) sink result \(sink.level)" }
            ...
            if overflowDraw > 0 {
                ...
                Debug.log(level: 179) { "digest.2d: source result \(source.level) sink result \(sink.level)" }
            }
        }

        Debug.log(level: 179) { "digest.4" }
        ...
        if fatToReady {
            ...
            if netDraw > 0 && netDeposit > 0 {
                Debug.log(level: 179) { "digest.6" }
                transfer(netDraw, from: fatStore, as: netDeposit, to: energy)
            }
        }

        Debug.log(level: 179) { "digest.7" }
    }
}
