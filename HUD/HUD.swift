...
class HUD {
    ...
    init(scene: SKScene) {
        ...
        let prototypesContainer = hardBind(scene.childNode(withName: "dashboard1"))
        ...
        let placeholdersContainer = hardBind(scene.childNode(withName: "dashboard2"))
    ...
    static private func unpackPlaceholders(_ dashboard: SKNode, scene: SKScene) -> [Quadrant] {
        let quadrants: [Quadrant] = (0..<4).map {
            ...
            let placeholder = hardBind(node as? SKSpriteNode)
    ...
    static private func unpackPrototypes(_ dashboard: SKNode, scene: SKScene) -> [Quadrant] {
        let quadrants: [Quadrant] = MonitorPrototype.allCases.map { monitorType in
            let name = hardBind(prototypeNames[monitorType])
            let node = dashboard.childNode(withName: name)
            let monitorPrototype = hardBind(node as? SKSpriteNode)
